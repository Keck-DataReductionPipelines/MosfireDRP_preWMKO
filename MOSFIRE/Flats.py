...
def tester(band):
        ...
        if band == 'Y':
                handle_flats([path % 3242, path % 3243, path % 3244], "npk_calib3_q1700_pa_0", band, options)

        if band == 'K':
                handle_flats([path % 3251, path % 3252, path % 3253], "npk_calib3_q1700_pa_0", band, options)

        if band == 'J':
                handle_flats([path % 3245, path % 3246, path % 3247], "npk_calib3_q1700_pa_0", band, options)
...
def tester3(band):
        options = Options.flat
        path = os.path.join(options["outdir"], "npk_calib3_q1700_pa_0", "combflat_2d_%s.fits" % band)
        res = np.load(os.path.join(path, "slit-edges_%s.npy" % band))
...
def handle_flats(flatlist, maskname, band, options):
        ...
        results[-1]["band"] = band
        np.save(os.path.join(options["outdir"], maskname, "slit-edges_%s" % band), results)
        save_ds9_edges(results, options)

        # Generate Flat
        out = os.path.join(options["outdir"], maskname, "pixelflat_2d_%s.fits" % (band))
...
def save_ds9_edges(results, options):
        ...
        band = results[-1]["band"]
        fn = os.path.join(options["outdir"], results[-1]["maskname"], "slit-edges_%s.ds9" % band)
        try:
                f = open(fn,'w')
                f.write(ds9)
                f.close()
...
def combine(flatlist, maskname, band, options):
        ...
        out = os.path.join(options["outdir"], maskname, "combflat_2d_%s.fits" % (band))
...
def fit_edge_poly(xposs, xposs_missing, yposs, widths, order):
        ...
        fit_edge_poly computes a parabola, and fills in missing data with a parabola
...
def find_and_fit_edges(data, ssl, options):
        ...
        Starting from the guess, find_edge_pair works out in either direction, measuring
        the position of the (e.g.) bottom of slit 1 and top of slit 2:
        ...
        for target in range(len(ssl) - 1):
                ...
                (xposs, xposs_missing, yposs, widths, scatters) = find_edge_pair(data, y, options["edge-fit-width"])
                (fun, wfun, res, sd, ok) = fit_edge_poly(xposs, xposs_missing, yposs, widths, options["edge-order"])
