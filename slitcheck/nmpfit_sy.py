...
-Modified by Sylvana Yelda (2008 May 6) to fix bug in the way the covariance matrix
was filled in (near line 1342).
"""
        ...
        if ((self.status > 0) and (nocovar==0) and (n != None)
            ...
            if ((n > 0) and (sz[0] >= n) and (sz[1] >= n)
                ...
                for i in range(n):
                    ...
                    # The line below is wrong! 'i' is an integer, which causes
                    # the covar matrix (cv) to not be filled in properly. Need
                    # to use 'indices' (above), which is an array.
